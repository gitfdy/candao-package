...
try {
  ...
  def url = 'https://git.can-dao.com/flutter-business/tappo.git'
...
'''], fallbackScript: [sandbox: true, script: "return ['Unable to read remote branches:disabled']"]))
    ...
    choice(name: 'ENVIRONMENT', choices: ['qc', 'beta', 'gray', 'release'], description: '业务环境，与分支独立；统一使用 Release 编译')
  ...
  environment {
    ...
    PROJECT = 'tappo'
    SOURCE_REPO = 'https://git.can-dao.com/flutter-business/tappo.git'
    DUFS_URL = 'http://192.168.225.46:5000/dufs/TAPPO'
